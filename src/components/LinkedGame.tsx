...
export default function LinkedGame({ puzzle, puzzleNumber }: Props) {
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-6 text-center">
        <div className="animate-[float_3s_ease-in-out_infinite]">
          <div className="grid grid-cols-2 gap-1.5 mb-8">
            {["bg-coral", "bg-teal", "bg-sky", "bg-amber"].map((c, i) => (
              <div
                key={i}
                className={`w-12 h-12 rounded-[10px] ${c} opacity-90`}
              />
            ))}
          </div>
        </div>
        ...
        <p className="text-text-muted text-base max-w-[340px] leading-relaxed mt-4 mb-10 animate-[fade-up_0.6s_ease_0.2s_forwards] opacity-0">
          Find four groups of four words that share a hidden link.
    ...
    >
      ...
      <div className="text-center mb-5 w-full max-w-[520px]">
        ...
        <p className="text-text-dim text-[13px] mt-1">
          Find the hidden link between four words
        </p>
      </div>
      ...
      {!gameOver && (
        <div
          className="grid grid-cols-4 gap-2 w-full max-w-[520px] mb-5"
          ...
          {remainingWords.map((word, i) => {
            ...
            return (
              <button
                ...
                className={`rounded-xl py-4.5 px-1 text-[13px] font-semibold tracking-wide
