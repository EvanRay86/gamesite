...
export default async function MathlerArchivePuzzlePage({ params }: Props) {
  ...
  let puzzle = await getMathlerPuzzleByDate(date);
  if (!puzzle) {
    puzzle = getMathlerPuzzle(date);
  }

  return (
    <main>
      <MathlerGame puzzle={puzzle} date={date} />
